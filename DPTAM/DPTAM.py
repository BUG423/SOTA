...
论文地址：MIE-Net: Motion Information Enhancement Network for Fine-Grained Action Recognition Using RGB Sensors 
...
class DPTAM(nn.Module):
    def __init__(self,in_channels,n_segment,kernel_size=3,stride=1,padding=1):
        ...
        self.dptam = nn.Sequential(nn.Conv1d(in_channels,in_channels // 4,kernel_size,stride=1,padding=kernel_size // 2,bias=False), nn.BatchNorm1d(in_channels // 4),
            nn.ReLU(inplace=True),nn.Conv1d(in_channels // 4, in_channels, 1, bias=False),nn.Sigmoid())
    def forward(self, x):
        nt, c, h, w = x.size()
        t = self.n_segment
        n_batch = nt // t
...
if __name__ == '__main__':
    test = DPTAM(6,8)# 实例化
    input = torch.randn(1024,6,32,32)# 创建一个随机输入张量，形状为[Batch, Channel, height, width]
